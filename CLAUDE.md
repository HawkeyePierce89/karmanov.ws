...
- **Testing scope:** only the pure logic modules in `src/lib/` (and `content.ts`) are unit-tested with Vitest — currently `lib/github.ts`, `lib/shapes.ts`, and `content.ts`. Component/markup correctness is verified via `npm run build` + `npm run check:seo`, not Vitest.
